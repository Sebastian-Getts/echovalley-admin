...
const generateMockData = (
  ...
  const baseData: StudentDetail[] = [
    { id: 's1', studentId: '2024001', name: '张三', class: '初一(1)班', score: 95, grade: '优秀', audioUrl: 'mock-audio-1' },
    { id: 's2', studentId: '2024002', name: '李四', class: '初一(1)班', score: 88, grade: '优秀', audioUrl: 'mock-audio-2' },
    { id: 's3', studentId: '2024003', name: '王五', class: '初一(1)班', score: 76, grade: '及格', audioUrl: 'mock-audio-3' },
    { id: 's4', studentId: '2024004', name: '赵六', class: '初一(1)班', score: 65, grade: '及格', audioUrl: 'mock-audio-4' },
    { id: 's5', studentId: '2024005', name: '钱七', class: '初一(1)班', score: 58, grade: '低分', audioUrl: 'mock-audio-5' },
    { id: 's6', studentId: '2024006', name: '孙八', class: '初一(2)班', score: 92, grade: '优秀', audioUrl: 'mock-audio-6' },
    { id: 's7', studentId: '2024007', name: '周九', class: '初一(2)班', score: 82, grade: '优秀', audioUrl: 'mock-audio-7' },
    { id: 's8', studentId: '2024008', name: '吴十', class: '初一(2)班', score: 70, grade: '及格', audioUrl: 'mock-audio-8' },
    { id: 's9', studentId: '2024009', name: '郑一', class: '初二(1)班', score: 55, grade: '低分', audioUrl: 'mock-audio-9' },
    { id: 's10', studentId: '2024010', name: '王二', class: '初二(1)班', score: 85, grade: '优秀', audioUrl: 'mock-audio-10' },
    { id: 's11', studentId: '2024011', name: '刘三', class: '初二(1)班', score: 78, grade: '及格', audioUrl: 'mock-audio-11' },
    { id: 's12', studentId: '2024012', name: '陈四', class: '初二(2)班', score: 91, grade: '优秀', audioUrl: 'mock-audio-12' },
    { id: 's13', studentId: '2024013', name: '杨五', class: '初二(2)班', score: 68, grade: '及格', audioUrl: 'mock-audio-13' },
    { id: 's14', studentId: '2024014', name: '黄六', class: '初三(1)班', score: 96, grade: '优秀', audioUrl: 'mock-audio-14' },
    { id: 's15', studentId: '2024015', name: '林七', class: '初三(1)班', score: 73, grade: '及格', audioUrl: 'mock-audio-15' },
  ]
  ...
  if (selectedQuestion !== 'all') {
    ...
    const adjustedData = baseData.map((student, index) => {
      ...
      const newGrade: '优秀' | '及格' | '低分' =
        newScore >= 90 ? '优秀' : newScore >= 70 ? '及格' : newScore >= 60 ? '及格' : '低分'
...
// 语音播放组件
function AudioPlayer({ audioUrl: _audioUrl, studentName }: { audioUrl: string; studentName: string }) {
...
export default function PracticeOverview() {
    ...
    () =>
      mockQuestions.filter((q) => {
        if (mode === 'exam') {
          ...
          return q.type !== 'exam' && (questionType === 'all' || q.type === questionType)
        }
      }),
    [mode, questionType],
  )
  ...
  const baseStudentDetails = useMemo(
    () => generateMockData(mode, questionType, selectedQuestion),
  ...
  const filteredAndSortedDetails = useMemo(() => {
    ...
    return result
  }, [filteredStudentDetails, detailFilterGrade, detailSearchKeyword, sortField, sortOrder])
  ...
  const totalDetailPages = Math.ceil(filteredAndSortedDetails.length / detailPageSize)

  // 计算统计数据（基于班级筛选后的数据）
  const totalStudents = filteredStudentDetails.length
  const excellentCount = filteredStudentDetails.filter((s) => s.grade === '优秀').length
  const passCount = filteredStudentDetails.filter((s) => s.grade === '及格').length
  const lowCount = filteredStudentDetails.filter((s) => s.grade === '低分').length

  const excellentRate = totalStudents > 0 ? ((excellentCount / totalStudents) * 100).toFixed(1) : '0.0'
  const passRate = totalStudents > 0 ? ((passCount / totalStudents) * 100).toFixed(1) : '0.0'
  const lowRate = totalStudents > 0 ? ((lowCount / totalStudents) * 100).toFixed(1) : '0.0'

  // 等级分布数据（用于饼图）
  const gradeDistribution = useMemo(
    () => [
      { name: '优秀', value: excellentCount, rate: parseFloat(excellentRate), color: '#10b981' },
      { name: '及格', value: passCount, rate: parseFloat(passRate), color: '#3b82f6' },
      { name: '低分', value: lowCount, rate: parseFloat(lowRate), color: '#f97316' },
    ],
  ...
  const scoreDistribution = useMemo(
    () => [
      ...
      {
        range: '60-70',
        count: filteredStudentDetails.filter((s) => s.score >= 60 && s.score < 70).length,
      },
      {
        range: '70-80',
        count: filteredStudentDetails.filter((s) => s.score >= 70 && s.score < 80).length,
      },
      {
        range: '80-90',
        count: filteredStudentDetails.filter((s) => s.score >= 80 && s.score < 90).length,
      },
      {
        range: '90-100',
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 12, color: '#6b7280' }}>
            等级分布
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie
                ...
                cy="50%"
                labelLine={false}
                label={(entry: any) => `${entry.name}: ${entry.rate}% (${entry.value}人)`}
                outerRadius={80}
                fill="#8884d8"
                dataKey="value"
              >
              ...
              <Tooltip
                formatter={(value: number, _name: string, props: any) => {
                  const payload = props.payload as { rate: number; name: string }
        ...
        >
          <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 12, color: '#6b7280' }}>成绩分布</div>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={scoreDistribution}>
              ...
              />
              <Bar dataKey="count" fill="#3b82f6" radius={[6, 6, 0, 0]} name="人数" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </section>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead style={{ backgroundColor: '#f9fafb' }}>
              <tr>
                ...
                >
                  学号 {sortField === 'studentId' && (sortOrder === 'asc' ? '↑' : '↓')}
                </th>
                <th
                  style={{
                    textAlign: 'left',
                ...
                >
                  班级 {sortField === 'class' && (sortOrder === 'asc' ? '↑' : '↓')}
                </th>
                <th
                  style={{
                    textAlign: 'right',
                ...
                >
                  得分 {sortField === 'score' && (sortOrder === 'asc' ? '↑' : '↓')}
                </th>
                <th style={{ textAlign: 'center', padding: '0.6rem 0.8rem', fontWeight: 600, width: '80px' }}>
                  等级
                </th>
                <th style={{ textAlign: 'center', padding: '0.6rem 0.8rem', fontWeight: 600, width: '80px' }}>
                  语音
              ...
              {paginatedDetails.length > 0 ? (
                paginatedDetails.map((student) => (
                  <tr key={student.id} style={{ borderTop: '1px solid #f3f4f6' }}>
                    <td style={{ padding: '0.6rem 0.8rem' }}>{student.studentId}</td>
                    <td style={{ padding: '0.6rem 0.8rem' }}>{student.name}</td>
                    <td style={{ padding: '0.6rem 0.8rem' }}>{student.class}</td>
                    <td style={{ padding: '0.6rem 0.8rem', textAlign: 'right', fontWeight: 500 }}>
                      {student.score} 分
                    </td>
                    ...
                    <td style={{ padding: '0.6rem 0.8rem', textAlign: 'center' }}>
                      <AudioPlayer audioUrl={student.audioUrl} studentName={student.name} />
                    </td>
                  </tr>
                ))
              ) : (
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 6,
                  border: '1px solid #e5e7eb',
                  backgroundColor: currentPage === totalDetailPages ? '#f9fafb' : '#ffffff',
                  color: currentPage === totalDetailPages ? '#9ca3af' : '#4b5563',
                  fontSize: 12,
                  cursor: currentPage === totalDetailPages ? 'not-allowed' : 'pointer',
                }}
              >
                下一页
              </button>
